        ...
        >
          <ul className="h-screen items-center justify-center md:flex md:h-auto">
            {navHeader.map((navItem, index) => (
              <li
                key={index}
                className="border-b-2 border-b-nightshade-blue py-4 text-center font-jost text-sm text-nightshade-blue hover:text-neutral-500 md:border-b-0 md:px-2 md:text-lg lg:px-6 lg:text-2xl"
              >
                <Link
                  href={`${navItem.link}`}
                  onClick={() => setIsNavBar(!isNavBar)}
